...
class AskRequest(BaseModel):
    city: str = Field(..., min_length=2, max_length=100, description="Tên thành phố bạn muốn xem thời tiết")
...
@app.post("/ask", response_model=AskResponse, tags=["Weather Agent"])
async def ask_agent(
    body: AskRequest,
    request: Request,
    ...
    answer = await get_weather(body.city)
    ...
    if r:
        ...
        r.rpush(hist_key, f"Q: Cập nhật thời tiết {body.city}")
    ...
    return AskResponse(
        question=f"Thời tiết {body.city} thế nào?",
        answer=answer,
